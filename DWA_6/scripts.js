...
// Usage of the abstracted functions

setInnerHTML('[data-list-button]', `
    <span>Show more</span>
    <span class="list__remaining"> (${(matches.length - (page * BOOKS_PER_PAGE)) > 0 ? (matches.length - (page * BOOKS_PER_PAGE)) : 0})</span>
`);

addEventListener('[data-search-cancel]', 'click', () => {
    getElement('[data-search-overlay]').open = false;
});

addEventListener('[data-header-search]', 'click', () => {
    getElement('[data-search-overlay]').open = true;
    getElement('[data-search-title]').focus();
});

addEventListener('[data-header-settings]', 'click', () => {
    getElement('[data-settings-overlay]').open = true;
});

addEventListener('[data-list-close]', 'click', () => {
    getElement('[data-list-active]').open = false;
});
